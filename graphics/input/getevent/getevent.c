...
#define GETEVENT_INFO(format, ...) \
  syslog(LOG_WARNING, "[getevent]: " format, ##__VA_ARGS__)
...
static int touch_init(FAR struct input_device_s *dev)
{
  ...
  if (!priv)
    {
      GETEVENT_INFO("Memory allocation failed for touch_priv_s\n");
      return -ENOMEM;
    }
...
{
  GETEVENT_INFO("Usage: getevent [OPTIONS]\n");
  GETEVENT_INFO("Options:\n");
  GETEVENT_INFO(" -h                Show this help message\n");
  GETEVENT_INFO(" -m /dev/mouse    -- mouse device path\n");
  GETEVENT_INFO(" -t /dev/input    -- touch device path\n");
  GETEVENT_INFO(" -k /dev/keyboard -- keyboard device path\n");
  GETEVENT_INFO("No options: Auto detect devices in /dev\n");
}
      ...
      if (entry->d_type == DT_CHR)
        {
          if (snprintf(path, sizeof(path), "/dev/%s", entry->d_name) < 0)
            {
              GETEVENT_INFO("Memory allocation failed for path\n");
              continue;
            }
