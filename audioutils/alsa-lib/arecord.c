/****************************************************************************
 * apps/audioutils/alsa-lib/arecord.c
 *
...
int main(int argc, FAR char *argv[])
{
  ...
  if (file_name)
    {
      fd = open(file_name, O_CREAT | O_WRONLY | O_BINARY);
      if (fd == -1)
        {
  ...
  while (frames_read < frames_limit)
    {
      ...
      frames_read += ret;
      if (write(fd, buffer, ret * frame_bytes) != ret * frame_bytes)
        {
